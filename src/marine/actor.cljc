(ns marine.actor
  "MarineEngineeringActor — the ISCO-08 3151 ship engineering coordination
  actor as a `langgraph.graph/state-graph` (ADR-2607011000). One graph run =
  one marine engineering operation (intake → advise → govern → decide →
  ...
  This actor coordinates vessel maintenance, fuel logistics, mechanical
  fault escalation, and engine parameter logging. It does NOT control engines,
  navigation, or command decisions (those remain the ship's exclusive human
  authority at sea).
  ...
  The unconditional invariant: the MarineAdvisor can never directly commit
  a record the MarineEngineeringGovernor refuses — every commit-record! call
  is gated behind `:decide`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [marine.advisor :as advisor]
            [marine.governor :as governor]
            [marine.store :as store]))

(defn build-graph
  "Build a compiled MarineEngineeringActor graph. `store` implements
  `marine.store/Store`. `advisor` implements `marine.advisor/Advisor`
  ...
  (-> (g/state-graph
      ...
      (g/add-node :commit
                   (fn [{:keys [request proposal]}]
                     (let [record {:vessel-id (:vessel-id proposal)
                                    :op (:op proposal)
                                    :payload proposal}]
